...
function formatItemName(item) {
    if (item.name === 'Groceries') return item.remarks; // For Option C search/display
    let name = item.name;
...
document.getElementById('groceryForm').onsubmit = async (e) => {
    ...
    // OPTION C implementation: Customer is 'Groceries', Remarks go into items list
        ...
        container.innerHTML = Object.entries(grouped).map(([name, custOrders]) => {
            ...
            <div class="p-6 bg-white rounded-3xl border shadow-sm border-l-8 border-pink-500">
                ...
                <div class="mt-3 pt-2 border-t flex justify-between items-center">
                    <span class="text-[10px] font-bold text-gray-400">Incl. RM ${deliveryTotal.toFixed(2)} Deliv.</span>
                    <span class="font-black text-pink-600">RM ${grandTotal.toFixed(2)}</span>
                </div>
            </div>`;
